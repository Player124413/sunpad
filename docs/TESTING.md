...
| 60 FPS | Physical-iPad stationary smoke passed eight samples at 59.8–59.9 FPS, 0.990–1.007 speed ratio, 2× scale, nominal thermal state, and Low Power Mode off; no-argument relaunch restored 30 FPS and save/preferences were unchanged; two StaticRecomp SMC hash mismatches were logged | Keep default off and restart-required with no UI; identify the two patched chunks, then verify real-time gameplay, physics, animation, cutscenes, audio, controller polling, save/reload, thermal/memory behavior, fallback/SMC counters, and a sustained physical-device session before any support claim |
