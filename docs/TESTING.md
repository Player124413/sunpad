...
| 60 FPS | A default-off **Experimental 60 FPS (Restart Required)** three-dot-menu option now persists the next-launch boot mode; original 30 FPS remains the default. A 14-minute-49-second physical-iPad telemetry pass held 59.7–60.0 FPS near real-time speed at 2× and nominal thermals except one recovered 42.1 FPS / 0.897 sample; the only two SMC demotions map exactly to the intentional Gecko code patches; returning to 30 FPS preserved save/preferences | Use the menu option and fully relaunch on physical iPad; accept hands-on gameplay speed, physics, animation, cutscenes, audio, controller polling, save/reload, memory behavior, and graceful-shutdown fallback/SMC counters before calling 60 FPS supported |
