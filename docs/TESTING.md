...
| 60 FPS | A 14-minute-49-second physical-iPad telemetry pass held 59.7–60.0 FPS near real-time speed at 2× and nominal thermals except one recovered 42.1 FPS / 0.897 sample; the only two SMC demotions map exactly to the intentional Gecko code patches; no-argument relaunch restored 30 FPS and save/preferences were unchanged | Keep default off and restart-required with no UI until hands-on gameplay speed, physics, animation, cutscenes, audio, controller polling, save/reload, memory behavior, and graceful-shutdown fallback/SMC counters pass |
